...
import me.salieri.lab4.DB.ADBClass;
import me.salieri.lab4.DB.DBClass;
...
public class DBUtils {
  ...
  public static void initialize(ADBClass db, int n) {
    db.create();
    db.clear();
    Scanner line;
    for (int i = 0; i < n; i++) {
  ...
  public static void execute(ADBClass db, String cmd) {
    if (cmd.charAt(0) != '/') {
    ...
    if (line.hasNext()) {
      ...
      if (!DBUtils.handlers.containsKey(key)) {
        System.out.println("Unknown command.");
        return;
      }

      try {
        DBUtils.handlers.get(key).invoke(db, line);
      } catch (Exception e) {
